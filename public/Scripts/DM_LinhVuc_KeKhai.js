...
function loadDataTable() {
    ...
    dt = $("#dataTable").DataTable({
        ...
        "processing": true,
        "ajax": {
            "url": "/DM_LinhVuc_KeKhai/LoadData",
            "type": "POST",
            "datatype": "json"
        },
        "columns": [
            ...
            { "data": "Ten_LinhVuc_KeKhai" },
            {
                "data": "Ma_LinhVuc_KeKhai", "render": function (data, type, row) {
                    return `
...
function FnBegin_Edit() {
    $('.loading_editlinhvuckekhai').removeClass('d-none');
}

function FnSuccess_Edit(data) {
    $('.loading_editlinhvuckekhai').addClass('d-none')
    $('.closeform').click()
...
function FnBegin() {
    $('.loading_newlinhvuckekhai').removeClass('d-none');
}

function FnSuccess(data) {
    $('.loading_newlinhvuckekhai').addClass('d-none');
    $('.closeform').click();
...
function Edit(obj) {
    ...
    var url = `/DM_LinhVuc_KeKhai/GetSuaLinhVucKeKhai/`
        ...
        .done(function (data) {
            ...
            var row1 = `
                              ...
                              <div class="form-group row">
                                <label for="cnsh" class="col-sm-3 col-form-label">Tên Lĩnh Vực Kê Khai</label>
                                <div class="col-sm-9">
                            <input type="hidden" disable="true" class="form-control" id="Ma_LinhVuc_KeKhai" name="Ma_LinhVuc_KeKhai" value="${data.Ma_LinhVuc_KeKhai}">
                                    <input type="text" class="form-control" placeholder="nhập tên lĩnh vực kê khai" id="Ten_LinhVuc_KeKhai" name="Ten_LinhVuc_KeKhai"  value="${data.Ten_LinhVuc_KeKhai}">
                                </div>
                            </div>
                                `
            $('#sua').prepend(row1);
...
function Delete(obj) {
    var ele = $(obj);
    var Ma_LinhVuc_KeKhai = ele.data("model-id");
    var url = `/DM_LinhVuc_KeKhai/delete/${Ma_LinhVuc_KeKhai}`
    swal.fire({
        title: 'Bạn có chắc?',
        ...
        if (result.isConfirmed) {
            $.post(url, { id: Ma_LinhVuc_KeKhai })
                .done(function (data) {

                    if (data != false) {
                        dt.rows(`#row_${data.Ma_LinhVuc_KeKhai}`)
                            .remove()
                            .draw();
                        Swal.fire({
                            icon: 'success',
